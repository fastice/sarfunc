...
def writeOldBase(insarInfo,b,db) :
    ...
    print('{0:11.8f} {1:11.4f} {2:11.4f}'.format(b[0],b[1],b[2]),file=fp)
    print('{0:11.8f} {1:11.8f} {2:11.8f}'.format(db[0],db[1],db[2]),file=fp)
...
def makeBaseParams(insarInfo,nAzLines,suffix='',logger=None) :  
    ...
    This was written originally for setupInt, but moved to its own routine so so it
    can be used by setupStrackReg when a params file is need for matching registration offsets'''
    #
    if logger != None :
    ...
    if not geodat.isRightLooking() :
        ...
        deltabn*=-1.0 
    # write params file
    fp=open(insarInfo['params']+suffix,'w')
    print('{0:11.4f} {1:11.4f} {2:11.4f} {3:7.1f} {4:11.4f}'.format(bn,bp,deltabn,0.0,deltabp),file=fp)
    fp.close()
    # write zero params file
    fp=open('zero.params','w')
